...
class MetaR(nn.Module):
    def __init__(self, dataset, parameter):
        ...
        if parameter['dataset'] == 'Wiki-One':
            ...
            self.relation_learner = RelationMetaLearner(parameter['few'], embed_size=400, num_hidden1=2000,
                                                        num_hidden2=800, out_size=400, dropout_p=self.dropout_p)
        elif parameter['dataset'] == 'HW':
            self.relation_learner = RelationMetaLearner(parameter['few'], embed_size=400, num_hidden1=2000,
                                                        num_hidden2=800, out_size=400, dropout_p=self.dropout_p)
    ...
    def forward(self, task, iseval=False, curr_rel=''):
        ...
        num_q = query.shape[1]              # num of query positive
        ...
        self.support_positive = support
        self.support_negative = support_negative
        self.query_positive = query
        self.query_negative = negative

        que_neg_e1, que_neg_e2 = self.split_concat(query, negative)  # [bs, nq+nn, 1, es]

        self.que_neg_e1 = que_neg_e1
        self.que_neg_e2 = que_neg_e2
